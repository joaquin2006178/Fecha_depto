...
with st.form("formulario_registro"):
    ...
    fecha_seleccionada = st.date_input("Selecciona una fecha", value=date.today(), min_value=fecha_inicio, max_value=fecha_fin, key="registro_fecha")
...
if "fechas_disponibles" not in locals():
    fechas_disponibles = []
...
dataframe_placeholder.dataframe(calendario, use_container_width=True)

# Opción para cancelar inscripciones
st.header("Cancelar Inscripción")
with st.form("formulario_cancelacion"):
    fecha_cancelar = st.date_input("Selecciona la fecha a cancelar", value=date.today(), min_value=fecha_inicio, max_value=fecha_fin, key="cancelacion_fecha")
    nombre_cancelar = st.text_input("Tu nombre para confirmar", max_chars=50, key="cancelacion_nombre")
